...
nestle.columns = [col[0] for col in nestle.columns]
nifty.columns = [col[0] for col in nifty.columns]
...
# Merge
data = nestle[['Close', 'Return']].join(nifty['Market_Return'], how='inner').dropna()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("nestle_capm_regression_plot.png")
plt.close()

# Step 3: ML Feature Engineering
data['Lag1'] = data['Return'].shift(1)
data['Lag2'] = data['Return'].shift(2)
data['MA5'] = data['Return'].rolling(5).mean()
data['Vol5'] = data['Return'].rolling(5).std()
...
plt.figure(figsize=(6, 4))
sns.barplot(x=importances, y=features)
plt.title("Nestlé - Random Forest Feature Importance")
plt.tight_layout()
plt.savefig("nestle_feature_importance_plot.png")
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("nestle_roc_curve_plot.png")
plt.close()

# Step 5: Value at Risk
var_95 = np.percentile(data['Return'], 5)
plt.figure(figsize=(8, 4))
sns.histplot(data['Return'], bins=50, kde=True)
plt.axvline(var_95, color='red', linestyle='--', label=f'95% VaR = {var_95:.2%}')
...
plt.legend()
plt.tight_layout()
plt.savefig("nestle_var_plot.png")
plt.close()

# Step 6: GARCH Forecast
garch_model = arch_model(data['Return'] * 100, vol='Garch', p=1, q=1)
garch_result = garch_model.fit(disp='off')
forecast = garch_result.forecast(horizon=5)
vol_forecast = forecast.variance[-1:].values.flatten()**0.5
...
# Output summary
print("Nestlé Financial ML Project Complete\n")
